...
def getFeatureVectors(trainingSubSet, testingSubSet):
    ...
    resnet18 = tv.models.resnet18(weights='IMAGENET1K_V1')
...
def reduceFeatureVectors(trainingFeatureVectors, testingFeatureVectors):
    ...
    pca.fit(trainingFeatureVectors)

    reducedTrainingFeatureVectors = pca.transform(trainingFeatureVectors)
    reducedTestingFeatureVectors = pca.transform(testingFeatureVectors)
...
def saveDataSets(trainingSet, testingSet, fileName):
    mainModel = {
        'trainingSet': trainingSet,
        'testingSet': testingSet
    }
    with open('../models/datasets/'+fileName+'.pkl', 'wb') as file:
        pickle.dump(mainModel, file)

def loadDataSets(fileName):
    with open('../models/datasets/'+fileName+'.pkl', 'rb') as file:
        mainModel = pickle.load(file)

    trainingSet = mainModel['trainingSet']
    testingSet = mainModel['testingSet']
    return trainingSet, testingSet

# Performs all Preprocessing EXCEPT feature vector extraction and reduction: load, transform
# Returns a list containing tuples for each data element (image, label)
def performPreprocessingImages():
    ...
    saveDataSets(trainingSet, testingSet, 'images')
...
def performPreprocessing():
    # Load Datasets (Keep first 500/class) AND Prepare DataSets for ResNet-18:
    trainingSet, testingSet = performPreprocessingImages()
    ...
    saveDataSets(finalTraining, finalTesting, 'featureVectors')
